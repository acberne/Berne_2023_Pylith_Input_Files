...
def GetPotDown(r,thet,phi,lmax,clm):

    vals=np.zeros((lmax,lmax))
    G=6.67e-11
    R=OuterRad
    delrho=rhoi  ##Ice-water density interface
    vals=np.zeros((lmax,lmax))
...
Pot20up=np.zeros((veclen,veclen))
Pot22up=np.zeros((veclen,veclen))

for i in range(veclen):
    for j in range(veclen):

        Pot20up[i,j]=GetPotDown(r,thet[i],phi[j],lmax,clm)[0,2]
        Pot22up[i,j]=GetPotDown(r,thet[i],phi[j],lmax,clm)[2,2]
        

##Need lower surface as well!



my_cols=['hash' ,'vtk' ,'DataFile' ,'Version' ,'2.0', 'hash2']

Vtkfile=pd.read_csv("InnerSurf_AddForce.vtk", 
                          names=my_cols,
                          sep=" ",
                          index_col=False)
df1=Vtkfile
...
def GetPotUp(r,thet,phi,lmax,clm):
    
    vals=np.zeros((lmax,lmax))
    G=6.67e-11
    R=InnerRad
    delrho=rhow-rhoi  ##Ice-water density interface
            
    for l in range(2,3):
        for m in range(l+1):
            if m==0:
                norm=clm[0,l,m]
                vals[m,l]=(delrho*(4*np.pi*G))*((norm)*(pysh.expand.spharm_lm(l,m,thet*180/np.pi,(phi*180/np.pi)-(((np.arctan2(clm[1,l,m],clm[0,l,m])*180/np.pi)))*(1/l)))*(1))*r*((R/r)**(l+2))*(1/(2*l+1))                  
            else:
                norm=np.linalg.norm(clm[:,l,m])
                vals[m,l]=(delrho*(4*np.pi*G))* ((norm)*(pysh.expand.spharm_lm(l,m,thet*180/np.pi,(phi*180/np.pi)-(((np.arctan2(clm[1,l,m],clm[0,l,m])*180/np.pi)))*(1/m)))*(1))*r*((R/r)**(l+2))*(1/(2*l+1))
                ##include (r/R)**l-2
    return vals
...
Pot20down=np.zeros((veclen,veclen))
Pot22down=np.zeros((veclen,veclen))

for i in range(veclen):
    for j in range(veclen):

        Pot20down[i,j]=GetPotUp(r,thet[i],phi[j],lmax,clm)[0,2]
        Pot22down[i,j]=GetPotUp(r,thet[i],phi[j],lmax,clm)[2,2]
        



##Okay, we also want the Actual Applied POtential
...
ne=(np.pi * 2)/T ##Mean motion in rad/s
InnerR=InnerRad 
OuterR=OuterRad ##Test radius, radius of Enceladus
...
for i in range(veclen):
    for j in range(veclen):
        ...
        Vn[i,j]=(-np.cos(omega*t)*(e/2)*(3/2)*(3*(np.cos(thet[i]))**2 -1)) 
...
numtest=1000000
k2_test=np.linspace(0,1.5,numtest)
...
k2=k2_test[np.argmin(Diff)]
    

file_object=open('k20_faulted.txt','a')
k2_str=str(k2)
file_object.write(k2_str)
file_object.write('\n')
file_object.close()
